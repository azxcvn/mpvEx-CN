    ...
    videoItems.isEmpty() -> {
      Box(modifier = modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(
          ...
          Text(text = "播放列表中没有视频", style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
          Text(text = "添加视频以开始", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
      }
    }
    else -> {
                ...
                Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                  ...
                  if (isReorderMode) {
                    IconButton(onClick = { }, modifier = Modifier.size(48.dp).draggableHandle()) {
                      Icon(imageVector = Icons.Filled.DragHandle, contentDescription = "拖动以重新排序", tint = MaterialTheme.colorScheme.primary)
                    }
                  }
                }
              }
...
private fun StreamUrlDialog(url: String, onDismiss: () -> Unit, onCopy: () -> Unit) {
  androidx.compose.material3.AlertDialog(
    ...
    dismissButton = { androidx.compose.material3.TextButton(onClick = onDismiss) { Text("关闭") } },
  )
}
...
private fun RemoveFromPlaylistDialog(isOpen: Boolean, onDismiss: () -> Unit, onConfirm: () -> Unit, itemCount: Int) {
  ...
  androidx.compose.material3.AlertDialog(
    onDismissRequest = onDismiss,
    title = { Text("从播放列表中移除 $itemCount $itemText？", style = MaterialTheme.typography.headlineMedium, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold) },
    text = {
      Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        androidx.compose.material3.Card(
          ...
          Text(
            text = "所选的 $itemText 将从此播放列表中移除。原始文件不会被删除。",
    ...
    confirmButton = {
      androidx.compose.material3.Button(
        ...
        shape = MaterialTheme.shapes.extraLarge,
      ) { Text(text = "从播放列表中移除", fontWeight = androidx.compose.ui.text.font.FontWeight.Bold) }
    },
    dismissButton = { androidx.compose.material3.TextButton(onClick = onDismiss, shape = MaterialTheme.shapes.extraLarge) { Text("取消", fontWeight = androidx.compose.ui.text.font.FontWeight.Medium) } },
